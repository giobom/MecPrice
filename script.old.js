/* ==============================
   MecPrice - Script principal (Atualizado)
   Orçamento + Estoque (em abas)
   - AppContext (Free/Pro)
   - gerarPDF() roteia Free/Pro
   - Modal Área PRO (UI)
============================== */
...
(() => {
  ...
  /* ==============================
     AppContext (preparado para login)
  ============================== */
  const AppContext = {
    plan: "free", // "free" | "pro"
    user: null,
  ...
  function renderEstoque() {
    ...
    if (msgEstoque) {
      ...
      msgEstoque.textContent = qtdBaixa ? `⚠️ ${qtdBaixa} item(ns) com estoque baixo.` : "Estoque OK ✅";
    }
  }
  ...
  function baixarEstoque(nomeDigitado, qtdUsada) {
    ...
    if (toNumber(it.qtd, 0) < qtd) {
      if (erroPeca) erroPeca.textContent = `Estoque insuficiente. Disponível: ${toNumber(it.qtd, 0)}`;
      return null;
    }
  ...
  function adicionarPeca() {
    ...
    if (!nome || Number.isNaN(qtd) || qtd <= 0 || Number.isNaN(valor) || valor < 0) {
      if (erroPeca) erroPeca.textContent = "Preencha nome, quantidade e valor unitário corretamente.";
      return;
    }
  ...
  function gerarPDFPro() {
    alert("PDF PRO ainda não implementado. (Quando tiver login, liberamos PDF com logo/dados da oficina.)");
  }
  ...
  function setupProModal() {
    ...
    // Placeholder: por enquanto só troca mensagem
    btnLogin?.addEventListener("click", () => {
      if (msg) msg.textContent = "Login PRO ainda não implementado. (Próximo passo: Supabase/Firebase)";
    });

    btnAssinar?.addEventListener("click", () => {
      if (msg) msg.textContent = "Assinatura PRO ainda não implementada. (Próximo passo: Mercado Pago/Stripe)";
    });

    // garante badge inicial
    setPlan("free");
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Mudança: PDF agora vai pelo roteador FREE/PRO
    ...
    // Estoque - ações por tabela (delegation)
